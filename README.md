...
3. Set Color Management Preferences to Legacy.

![preview](images/color_management_preferences.png)

4. Save the preferences and restart MAYA.

5. Create `DirectX 11 Shader` material.
...
6. Open the material's attributes and change the shader file to the `MayaToonOutline.fx` file you downloaded.

![preview](images/change_shader_file.png)


7. Apply the material to the object.
8. Enable `Textured` for a real-time preview.
...
| Shadow Filter Width | Blur the shadow boundary. |
| Shadow Filter Count | Specify the number of times to draw shadows. A higher number of draws will result in smoother shadows. |
...
| Parameters | Description |
| --- | --- |
| Detail Color Map UV | Select the UV for the Detail Color Map. If you don't want to use the Detail Color Map, select `None`. |
| Detail Color Map | Specifies the Detail Color Map Texture. |
